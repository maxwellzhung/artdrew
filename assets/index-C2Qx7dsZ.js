...
`;var we=document.querySelector(`#resultGrid`),Te=document.querySelector(`#collectionGrid`),Ee=document.querySelector(`#recentGrid`),P=document.querySelector(`#blindBox`),F=document.querySelector(`#boxStage`),De=document.querySelector(`#pityCount`),Oe=document.querySelector(`#pityRemain`),ke=document.querySelector(`#heroCompletion`),Ae=document.querySelector(`#drawCompletion`),je=document.querySelector(`#heroWish`),Me=document.querySelector(`#wishName`),Ne=document.querySelector(`#heroStock`),Pe=document.querySelector(`#drawStock`),Fe=document.querySelector(`#machineStock`),Ie=document.querySelector(`#walletInk`),Le=document.querySelector(`#walletToken`),Re=document.querySelector(`#walletCompletion`),ze=document.querySelector(`#drawStatus`),Be=document.querySelector(`#resultSummary`),Ve=document.querySelector(`#collectionSummary`),He=document.querySelector(`#wishCard`),Ue=document.querySelector(`#oddsGrid`),I=document.querySelector(`#loopPanel`),We=document.querySelector(`#shelfPreview`),Ge=document.querySelector(`#dropBoard`),Ke=document.querySelector(`#emotionMap`),qe=document.querySelector(`#forgeMain`),Je=document.querySelector(`#forgeUniverse`),Ye=document.querySelector(`#forgePinned`),Xe=document.querySelector(`#forgeCharacters`),Ze=document.querySelector(`#forgeStatus`),Qe=document.querySelector(`#forgeGenerateButton`),L=document.querySelector(`#forgePinButton`),$e=document.querySelector(`#forgeCopyButton`),et=document.querySelectorAll(`[data-filter]`),tt=document.querySelectorAll(`[data-language]`),nt=document.querySelectorAll(`.nav-links a`),R=document.querySelector(`#drawOneButton`),z=document.querySelector(`#drawTenButton`),B=document.querySelector(`#leverButton`),rt=document.querySelector(`#resetButton`),V=document.querySelector(`#twilloopWishButton`),H=document.querySelector(`#twilloopWishState`),it=document.querySelector(`#accountButton`),at=document.querySelector(`#accountLabel`),ot=document.querySelector(`#accountIntro`),U=document.querySelector(`#accountPrimaryAction`),W=document.querySelector(`#accountSecondaryAction`),st=document.querySelector(`#accountDashboard`),G=document.querySelector(`#authModal`),ct=document.querySelector(`#authDialog`),lt=t=>e.find(e=>e.id===t),K=()=>lt(y)??e[e.length-1],q=()=>Object.values(g).reduce((e,t)=>e+t,0),J=()=>e.filter(e=>(g[e.id]??0)>0).length,ut=()=>Math.max(0,pe+b.stockBoost-q()),dt=()=>Math.round(J()/e.length*100),Y=e=>e.replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]??e),ft=()=>`AD-${new Date().getFullYear()}-${Math.floor(1e3+Math.random()*9e3)}`,pt=e=>e.trim().replace(/\s+/g,``),mt=e=>{let t=e.trim();return t?(t.split(`@`)[0]||t).slice(0,12):a===`zh`?`artdrew 玩家`:`artdrew player`},ht=()=>E?J()>=5||q()>=18?a===`zh`?`雾光藏家`:`Mistlight Collector`:J()>=3||q()>=8?a===`zh`?`雨后会员+`:`After-Rain Member+`:a===`zh`?`雨后会员`:`After-Rain Member`:a===`zh`?`游客`:`Guest`,gt=()=>E?(E.displayName.trim().replace(/\s+/g,``).slice(0,2)||`AD`).toUpperCase():`${J()}/${e.length}`,_t=e=>{D=e,O=!0,k=o(e===`register`?s.account.registerMessage:s.account.loginMessage),Q()},vt=()=>{O=!1,Q()},yt=(e,t)=>{let n=pt(t),r=e.trim()||mt(n);E={displayName:r,handle:`@${r.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fa5]+/g,``).slice(0,16)||`artdrew`}`,contact:n||`demo@artdrew.local`,memberNo:ft(),createdAt:new Date().toLocaleDateString(a===`zh`?`zh-CN`:`en-US`,{month:`2-digit`,day:`2-digit`})},O=!1,k=`${o(s.account.profileCreatedPrefix)}${E.displayName}${o(s.account.profileCreatedSuffix)}`,T=`${o(s.account.signedInPrefix)}${E.displayName}${o(s.account.signedInSuffix)}`,window.location.hash=`#account`,$(),Q()},bt=()=>{if(v>=m-1)return e.find(e=>e.rarity===`SECRET`)??e[0];let t=K();if(b.wishSparks>=me-1&&t.rarity!==`SECRET`)return t;let n=Math.random()*100,r=0;for(let t of e)if(r+=t.probability,n<=r)return t;return e[0]},xt=e=>{if(C)return;let t=ut(),n=Math.min(e,t);if(n<=0){T=o(s.draw.statusSoldOut),Q();return}C=!0,x=[],w=null,T=`${o(s.draw.statusOpeningPrefix)} ${n} ${o(s.draw.statusOpeningSuffix)}`,Q(),P?.classList.remove(`reveal`),P?.classList.add(`shake`),window.setTimeout(()=>{P?.classList.remove(`shake`),P?.classList.add(`reveal`);for(let e=0;e<n;e+=1){let e=bt(),t=(g[e.id]??0)>0;x.push(e),t&&(b.duplicateInk+=e.rarity===`SECRET`?5:e.rarity===`RARE`?2:1),b.wishSparks=e.id===y?0:Math.min(me,b.wishSparks+1),g[e.id]=(g[e.id]??0)+1,_.push(e.id),v=e.rarity===`SECRET`?0:v+1}localStorage.setItem(ue,String(v)),xe(g),Se(_),h(),C=!1;let e=K(),t=x.filter(t=>t.id===e.id).length,r=x.filter(e=>e.rarity===`SECRET`).length,i=x.filter(e=>e.rarity===`RARE`).length;w=r?`SECRET`:t?`WISH`:i?`RARE`:`COMMON`;let c=p(e);T=t?a===`zh`?`${o(s.draw.statusWishHit)} ${t} 次：${c.title}。`:`${o(s.draw.statusWishHit)} ${t} time${t>1?`s`:``}: ${c.title}.`:r?o(s.draw.statusSecret):i?`${o(s.draw.statusRare)} ${i} ${o(s.draw.statusRareSuffix)}`:o(s.draw.statusCommon),Q()},520)},St=(e,n=0,r=!1)=>{let i=e.id===y,c=p(e);return`
    <article class="figure-card ${t[e.rarity]} ${r?`compact`:``} ${n>0?`owned`:`locked`} ${i?`wish`:``}" style="--figure-color: ${e.color}">
      ...
      <div>
        <div class="card-line">
          <h3>${c.title}</h3>
          <span class="rarity ${t[e.rarity]}">${f(e.rarity)}</span>
        </div>
        <p>${c.subtitle}</p>
...
`,wt=()=>{if(!Be)return;if(!x.length){Be.textContent=o(s.draw.resultEmpty);return}let e=x.reduce((e,t)=>(e[t.rarity]+=1,e),{COMMON:0,RARE:0,SECRET:0});Be.textContent=`${o(s.labels.common)} ${e.COMMON} / ${o(s.labels.rare)} ${e.RARE} / ${o(s.labels.secret)} ${e.SECRET}`},Tt=()=>{we&&(we.innerHTML=x.length?x.map(e=>St(e,g[e.id],!0)).join(``):Ct(),wt())},Et=e=>{let t=(g[e.id]??0)>0;return S===`owned`?t:S===`missing`?!t:S===`common`?e.rarity===`COMMON`:S===`rare`?e.rarity===`RARE`:S===`secret`?e.rarity===`SECRET`:!0},Dt=()=>{et.forEach(e=>{let t=e.dataset.filter===S,n=e.dataset.filter;e.classList.toggle(`active`,t),e.setAttribute(`aria-pressed`,String(t)),n&&s.collection.filters[n]&&(e.textContent=o(s.collection.filters[n]))})},Ot=()=>{if(!Te)return;let t=e.filter(Et);Te.innerHTML=t.length?t.map(e=>St(e,g[e.id]??0)).join(``):`<div class="empty-result collection-empty"><span>0</span><p>${o(s.draw.emptyCollection)}</p></div>`},kt=()=>{We&&(We.innerHTML=e.map(e=>{let n=g[e.id]??0,r=p(e);return`
        <article class="shelf-item ${t[e.rarity]} ${n>0?`owned`:`locked`}" style="--toy-color: ${e.color}">
          ...
          <small>${n>0?`x${n}`:f(e.rarity)}</small>
        </article>
      `}).join(``))},At=()=>{Ge&&(Ge.innerHTML=n.map(e=>{let t=ee(e);return`
      ...
      `}).join(``))},jt=()=>{let e=ne(),n=j.some(t=>t.id===e.id);Ze&&(Ze.textContent=M||o(s.forge.ready)),Qe&&(Qe.textContent=o(s.forge.generate)),L&&(L.textContent=o(n?s.forge.pinned:s.forge.pin),L.disabled=n),$e&&($e.textContent=o(s.forge.copy)),qe&&(qe.innerHTML=`
      ...
      ${N?`
            <div class="forge-export">
              ...
              <textarea id="forgeExportTextarea" readonly spellcheck="false">${Y(N)}</textarea>
            </div>
          `:``}
    `),Je&&(Je.innerHTML=`
    ...
    `),Xe&&(Xe.innerHTML=e.figures.map(e=>`
          <article class="forge-character ${t[e.rarity]}" style="--forge-color:${e.color}">
            <div class="forge-character-top">
              ...
              <div>
                <span class="rarity ${t[e.rarity]}">${f(e.rarity)} / ${e.probability}%</span>
            ...
            <div class="forge-bio-grid">
              ...
              <span><strong>${o(s.forge.visualHooks)}</strong>${ie(e.visualHooks).join(` / `)}</span>
            </div>
          </article>
        `).join(``))},Mt=()=>{if(!He)return;let e=K(),t=g[e.id]??0,n=p(e);He.innerHTML=`
  ...
  `},Nt=()=>{Ue&&(Ue.innerHTML=e.map(e=>{let n=p(e);return`
        <article class="odds-card ${t[e.rarity]} ${e.id===y?`wish`:``}">
          <div class="card-line">
            <h3>${n.title}</h3>
            <span class="rarity ${t[e.rarity]}">${f(e.rarity)}</span>
          </div>
      ...
      `}).join(``))},Pt=()=>{Ke&&(Ke.innerHTML=e.map(e=>{let n=p(e);return`
      ...
      `}).join(``))},Ft=(e,t)=>Math.min(100,Math.round(Math.min(e,t)/t*100)),It=(e,t,n,r=``)=>`
  <div class="loop-progress ${r}" aria-label="${n} ${Math.min(e,t)} / ${t}">
    <span style="width: ${Ft(e,t)}%"></span>
  </div>
`,Lt=()=>[{id:`first-draw`,title:o(s.loop.firstTitle),detail:o(s.loop.firstDetail),target:1,reward:1,progress:q()},{id:`ten-draw`,title:o(s.loop.tenTitle),detail:o(s.loop.tenDetail),target:10,reward:3,progress:q()},{id:`three-unlocks`,title:o(s.loop.unlockTitle),detail:o(s.loop.unlockDetail),target:3,reward:2,progress:J()},{id:`set-wish`,title:o(s.loop.wishMissionTitle),detail:o(s.loop.wishMissionDetail),target:1,reward:1,progress:+!!b.wishSet},{id:`duplicate-ink`,title:o(s.loop.duplicateTitle),detail:o(s.loop.duplicateDetail),target:3,reward:2,progress:b.duplicateInk}],Rt=()=>{if(!I)return;let e=q(),t=K(),n=p(t),r=Math.max(0,m-v),i=t.rarity===`SECRET`?o(s.loop.wishSecret):`${Math.max(0,me-b.wishSparks)} ${o(s.loop.wishTrigger)}`,a=Lt(),c=b.claimedMissions.length,l=b.tokens<he,u=e>=30?o(s.loop.restDone):`${o(s.loop.restMore)} ${30-e} ${o(s.loop.restMoreTail)}`;I.innerHTML=`
    <div class="loop-head">
      <div>
      ...
      <div class="loop-wallet" aria-label="${o(s.loop.walletAria)}">
        <span><strong>${b.tokens}</strong> ${o(s.labels.tokens)}</span>
        <span><strong>${b.duplicateInk}</strong> ${o(s.labels.duplicateInk)}</span>
    ...
    <div class="loop-grid">
      <article class="loop-card">
        ...
        <p>${o(s.loop.pityCopy)} ${v} ${o(s.loop.pityCopyTail)}</p>
        ${It(v,m,`${o(s.loop.pityTitle)} ${o(s.loop.progress)}`,`secret`)}
      </article>
    ...
    <div class="mission-list" aria-label="${o(s.loop.todayMissions)}">
      ${a.map(e=>{let t=b.claimedMissions.includes(e.id),n=e.progress>=e.target;return`
            <article class="mission-row ${t?`claimed`:``}">
              <div>
                <h4>${e.title}</h4>
                <p>${e.detail}</p>
                ${It(e.progress,e.target,`${e.title} ${o(s.loop.progress)}`)}
              </div>
  ...
  `},zt=()=>{if(!Ee)return;let e=_.slice(-18).reverse().map(lt).filter(e=>!!e);Ee.innerHTML=e.length?e.map(e=>{let n=p(e);return`
            <span class="recent-chip ${t[e.rarity]}" style="--toy-color: ${e.color}">
              ${n.title}
            </span>
          `}).join(``):`<p class="muted-line">${o(s.collection.noHistory)}</p>`},Bt=()=>{if(!Ve)return;let t=q(),n=J(),r=(g[`blackout-bloom`]??0)>0;Ve.innerHTML=`
  ...
  `},Vt=()=>{let n=J(),r=q(),i=K(),a=ht();if(it?.classList.toggle(`signed-in`,!!E),at&&(at.textContent=E?E.displayName:o(s.account.login)),ot&&(ot.textContent=o(E?s.account.introUser:s.account.introGuest)),U&&(U.textContent=o(E?s.account.viewShelf:s.account.create),U.toggleAttribute(`data-auth-open`,!E),E||(U.dataset.authOpen=`register`),U.toggleAttribute(`data-account-jump`,!!E),E&&(U.dataset.accountJump=`#collection`)),W&&(W.textContent=o(E?s.account.logout:s.account.login),W.toggleAttribute(`data-auth-open`,!E),E||(W.dataset.authOpen=`login`),W.toggleAttribute(`data-sign-out`,!!E)),!st)return;let c=_.slice(-5).reverse().map(lt).filter(e=>!!e),l=c.length?c.map(e=>{let n=p(e);return`<span class="recent-chip ${t[e.rarity]}">${n.title}</span>`}).join(``):`<p class="muted-line">${o(s.collection.noHistory)}</p>`,u=p(i);if(!E){st.innerHTML=`
    ...
    `;return}let d=Y(E.displayName),f=Y(E.handle),ee=Y(E.contact),te=Y(E.memberNo);st.innerHTML=`
    <article class="account-panel profile-panel">
      ...
      <dl class="profile-list">
        <div><dt>${o(s.account.memberNo)}</dt><dd>${te}</dd></div>
        <div><dt>${o(s.account.contact)}</dt><dd>${ee}</dd></div>
        <div><dt>${o(s.account.joined)}</dt><dd>${E.createdAt}</dd></div>
      </dl>
    </article>
    ...
    <article class="account-panel">
      ...
      <div class="account-progress" aria-label="${o(s.account.progressAria)} ${dt()}%">
        <span style="width: ${dt()}%"></span>
      </div>
      <p class="account-mini-copy">${o(s.account.currentWish)}${u.title}${o(s.account.pityLeft)} ${Math.max(0,m-v)}${o(s.account.pityUnit)}</p>
    </article>
  ...
  `},Ht=()=>{if(!G||!ct)return;if(G.classList.toggle(`hidden`,!O),!O){ct.innerHTML=``;return}let e=D===`register`;ct.innerHTML=`
    <div class="auth-head">
      <div>
    ...
    <div class="auth-tabs" role="tablist" aria-label="${o(s.account.authTabs)}">
      <button type="button" data-auth-mode="login" aria-selected="${D===`login`}">${o(s.account.login)}</button>
      <button type="button" data-auth-mode="register" aria-selected="${D===`register`}">${o(s.account.registerTab)}</button>
    </div>
    <form class="auth-form" id="authForm">
      ...
      <p class="auth-message">${k}</p>
    </form>
  `},X=(e,t)=>{let n=document.querySelector(e);n&&(n.textContent=t)},Z=(e,t)=>{let n=document.querySelector(e);if(!n)return;let r=Array.from(n.childNodes).find(e=>e.nodeType===Node.TEXT_NODE);if(r){r.textContent=` ${t}`;return}n.append(document.createTextNode(` ${t}`))},Ut=(e,t)=>{let n=document.querySelector(e)?.nextElementSibling;n instanceof HTMLParagraphElement&&(n.textContent=t)},Wt=()=>{let e=document.querySelector(`.twilloop-card-grid`);e&&(e.innerHTML=r.map(e=>`
        <article class="twilloop-card ${e.rarity.toLowerCase()}" style="--twilloop-color: ${e.color}">
          <span>${e.rarity===`COMMON`?o(s.labels.common):e.rarity===`RARE`?o(s.labels.rare):o(s.labels.secret)}</span>
          <h4>${a===`zh`?e.cnName:e.name}</h4>
          <p>${a===`zh`?e.name:e.cnName}</p>
          <small>${te(e.name,e.trait)}</small>
        </article>
      `).join(``))},Gt=()=>{document.documentElement.lang=a===`zh`?`zh-CN`:`en`,document.title=o(s.meta.title),document.querySelector(`meta[name="description"]`)?.setAttribute(`content`,o(s.meta.description)),tt.forEach(e=>{let t=e.dataset.language===a;e.classList.toggle(`active`,t),e.setAttribute(`aria-pressed`,String(t))});let e=[`top`,`drops`,`forge`,`twilloop`,`draw`,`story`,`collection`,`odds`,`account`];nt.forEach((t,n)=>{let r=e[n];r&&(t.textContent=o(s.nav[r]))}),document.querySelector(`.nav-actions`)?.setAttribute(`aria-label`,a===`zh`?`玩家资产`:`Player assets`);let t=document.querySelectorAll(`.wallet-plus`);t[0]?.setAttribute(`aria-label`,a===`zh`?`重复水珠说明`:`Duplicate ink info`),t[1]?.setAttribute(`aria-label`,a===`zh`?`余光章说明`:`Afterglow token info`),it?.setAttribute(`aria-label`,o(E?s.account.viewShelf:s.account.login)),X(`.hero-cn`,o(s.hero.nameLine)),X(`.hero-slogan`,o(s.hero.slogan)),X(`.hero-lede`,o(s.hero.lede)),X(`.hero-actions .button.primary`,o(s.hero.drawAction)),X(`.hero-actions .button.ghost`,o(s.hero.dropsAction)),Z(`.drop-stats span:nth-child(1)`,o(s.hero.statsCommon)),Z(`.drop-stats span:nth-child(2)`,o(s.hero.statsRare)),Z(`.drop-stats span:nth-child(3)`,o(s.hero.statsSecret)),Z(`.drop-stats span:nth-child(4)`,o(s.hero.statsCollected)),Z(`.drop-stats span:nth-child(5)`,o(s.hero.statsStock)),X(`.announce-strip span`,o(s.hero.announce));let n=document.querySelector(`.announce-strip p`);n?.firstChild?.nodeType===Node.TEXT_NODE&&(n.firstChild.textContent=o(s.hero.announceText)),X(`.drop-board-head h2`,o(s.drops.title)),X(`.drop-board-head > p`,o(s.drops.intro)),X(`.forge-copy h2`,o(s.forge.title)),X(`.forge-copy p:not(.eyebrow)`,o(s.forge.intro)),document.querySelector(`.forge-controls`)?.setAttribute(`aria-label`,a===`zh`?`IP 生成器操作`:`IP generator actions`),X(`.forge-character-head h3`,o(s.forge.characterMatrix)),X(`.forge-character-head > p`,o(s.forge.prototypeOnly)),X(`.twilloop-cn`,o(s.twilloop.titleLine)),X(`.twilloop-copy .twilloop-slogan`,`Loose threads, tiny repair rituals.`),Ut(`.twilloop-copy .twilloop-slogan`,o(s.twilloop.lede)),X(`.twilloop-actions .button.primary`,o(s.twilloop.view)),V&&(V.textContent=o(i?s.twilloop.wished:s.twilloop.wish)),H&&(H.textContent=o(i?s.twilloop.wishedState:s.twilloop.idle));let r=document.querySelectorAll(`.twilloop-specs span`);[s.twilloop.specA,s.twilloop.specB,s.twilloop.specC].forEach((e,t)=>{r[t]&&(r[t].textContent=o(e))}),X(`.twilloop-lineup-copy h3`,o(s.twilloop.lineupTitle)),X(`.twilloop-lineup-copy p:not(.eyebrow)`,o(s.twilloop.lineupCopy)),Wt(),X(`.draw-section .section-heading h2`,o(s.draw.title)),Z(`.section-meter span:nth-child(1)`,o(s.draw.completion)),Z(`.section-meter span:nth-child(2)`,o(s.draw.stock)),Z(`.section-meter span:nth-child(3)`,o(s.draw.wish)),Z(`.section-meter span:nth-child(4)`,o(s.draw.pity)),X(`.machine-rules span`,o(s.draw.rules)),X(`.machine-rules p`,o(s.draw.ruleText)),X(`.machine-rules small`,o(s.draw.stockSmall)),B?.setAttribute(`aria-label`,o(s.draw.lever)),Z(`.stock-line`,o(s.draw.noSecret)),X(`.result-panel .panel-head span`,o(s.draw.resultTitle)),X(`.result-panel .panel-link`,o(s.draw.allRecords)),rt?.setAttribute(`aria-label`,o(s.draw.reset)),X(`.shelf-preview .panel-head span`,o(s.draw.shelfTitle)),X(`.shelf-preview .panel-head p`,o(s.draw.shelfCopy)),X(`.shelf-preview .panel-link`,o(s.draw.viewAll)),I?.setAttribute(`aria-label`,o(s.loop.title)),X(`.series-copy h2`,o(s.story.seriesTitle)),X(`.series-copy p:not(.eyebrow)`,o(s.story.seriesCopy)),X(`.story-intro h2`,o(s.story.bibleTitle)),X(`.story-intro p:not(.eyebrow)`,o(s.story.bibleCopy));let c=document.querySelectorAll(`.story-pillars article`);[[s.story.originTitle,s.story.originCopy],[s.story.desireTitle,s.story.desireCopy],[s.story.collectTitle,s.story.collectCopy]].forEach(([e,t],n)=>{c[n]?.querySelector(`h3`)?.replaceChildren(document.createTextNode(o(e))),c[n]?.querySelector(`p`)?.replaceChildren(document.createTextNode(o(t)))});let l=document.querySelectorAll(`.rarity-strip span`);[s.odds.common,s.odds.rare,s.odds.secret,s.odds.pity].forEach((e,t)=>{l[t]&&(l[t].textContent=o(e))}),X(`.collection-section .section-heading h2`,o(s.collection.title)),document.querySelector(`.collection-tools`)?.setAttribute(`aria-label`,a===`zh`?`藏品筛选`:`Collection filters`),X(`.recent-panel .panel-head span`,o(s.collection.recentTitle)),X(`.recent-panel .panel-head p`,o(s.collection.recentCopy)),X(`.account-hero-card h2`,o(s.account.title)),document.querySelector(`.auth-backdrop`)?.setAttribute(`aria-label`,o(s.account.closeWindow))},Kt=()=>{let t=J(),n=dt(),r=ut(),i=Math.max(0,30-v),c=p(K());De&&(De.textContent=String(v)),Oe&&(Oe.textContent=String(i)),ke&&(ke.textContent=`${t}/${e.length}`),Ae&&(Ae.textContent=`${n}%`),je&&(je.textContent=c.title),Me&&(Me.textContent=c.title),Ne&&(Ne.textContent=String(r)),Pe&&(Pe.textContent=String(r)),Fe&&(Fe.textContent=String(r)),Ie&&(Ie.textContent=String(b.duplicateInk)),Le&&(Le.textContent=String(b.tokens)),Re&&(Re.textContent=gt(),Re.setAttribute(`aria-label`,E?a===`zh`?`当前账户 ${E.displayName}`:`Current account ${E.displayName}`:a===`zh`?`已解锁 ${t} 款，共 ${e.length} 款`:`${t} of ${e.length} figures unlocked`)),ze&&(ze.textContent=T),F&&(F.classList.toggle(`celebrate`,!!w),F.classList.toggle(`tone-common`,w===`COMMON`),F.classList.toggle(`tone-rare`,w===`RARE`),F.classList.toggle(`tone-secret`,w===`SECRET`),F.classList.toggle(`tone-wish`,w===`WISH`));let l=r<=0;R&&(R.disabled=C||l,R.textContent=o(C?s.draw.drawing:s.draw.one)),z&&(z.disabled=C||l,z.textContent=o(l?s.draw.soldOut:s.draw.ten)),B&&(B.disabled=C||l,B.classList.toggle(`pulling`,C))},Q=()=>{Gt(),Kt(),At(),jt(),Tt(),Dt(),Ot(),kt(),Mt(),Nt(),Pt(),Rt(),zt(),Bt(),Vt(),Ht()},qt=e=>_e.some(t=>t.id===e),Jt=e=>{let t=e?.closest(`[data-auth-open]`),n=e?.closest(`[data-account-jump]`),r=e?.closest(`[data-sign-out]`);if(t?.dataset.authOpen===`login`||t?.dataset.authOpen===`register`)return _t(t.dataset.authOpen),!0;if(n?.dataset.accountJump)return window.location.hash=n.dataset.accountJump,$(),!0;if(r){let e=E?.displayName??(a===`zh`?`账户`:`Account`);return E=null,O=!1,k=`${e}${o(s.account.signedOutSuffix)}`,T=o(s.account.signedOutStatus),Q(),!0}return!1},Yt=e=>{a!==e&&(a=e,T=o(s.draw.statusIdle),M=``,N=``,E||(k=o(O?D===`register`?s.account.registerMessage:s.account.loginMessage:s.account.guestMessage)),Q())};tt.forEach(e=>{e.addEventListener(`click`,()=>{(e.dataset.language===`zh`||e.dataset.language===`en`)&&Yt(e.dataset.language)})}),it?.addEventListener(`click`,()=>{if(E){window.location.hash=`#account`,$();return}_t(`login`)}),U?.addEventListener(`click`,e=>{Jt(e.currentTarget)}),W?.addEventListener(`click`,e=>{Jt(e.currentTarget)}),Qe?.addEventListener(`click`,()=>{re(),M=o(s.forge.generated),N=``,T=M,Q()}),L?.addEventListener(`click`,()=>{let e=ne();if(j.some(t=>t.id===e.id)){M=o(s.forge.pinDuplicate),Q();return}j=[...j,e],M=o(s.forge.pinSaved),T=M,Q()}),$e?.addEventListener(`click`,()=>{let e=oe(ne());N=e,se(e).then(e=>{M=o(e?s.forge.copied:s.forge.exportReady),T=M,Q()}).catch(()=>{M=o(s.forge.exportReady),T=M,Q()})}),Ye?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-forge-load]`)?.dataset.forgeLoad,n=t?d.findIndex(e=>e.id===t):-1;n<0||(A=n,M=a===`zh`?`已回看暂存概念。`:`Pinned concept loaded.`,N=``,Q())}),G?.addEventListener(`click`,e=>{let t=e.target;if(t.closest(`[data-auth-close]`)){vt();return}let n=t.closest(`[data-auth-mode]`);(n?.dataset.authMode===`login`||n?.dataset.authMode===`register`)&&(D=n.dataset.authMode,k=o(D===`register`?s.account.registerMessage:s.account.loginMessage),Q())}),G?.addEventListener(`submit`,e=>{let t=e.target.closest(`#authForm`);if(!t)return;e.preventDefault();let n=new FormData(t),r=String(n.get(`contact`)??``).trim(),i=String(n.get(`displayName`)??``).trim();if(!r){k=o(s.account.needContact),Q();return}yt(i,r)}),window.addEventListener(`keydown`,e=>{e.key===`Escape`&&O&&vt()}),R?.addEventListener(`click`,()=>xt(1)),z?.addEventListener(`click`,()=>xt(10)),B?.addEventListener(`click`,()=>xt(1)),Ge?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-drop-target]`);if(!t?.dataset.dropTarget)return;let r=n.find(e=>e.id===t.dataset.dropTarget);r?.href||(T=r?.id===`morrifolio`?a===`zh`?`Morrifolio / 墨里弗里已保留为 artdrew 上架档案；当前 LIVE 抽盒是 Brumlet / 雾姆团。`:`Morrifolio is preserved as an artdrew release archive; the current live draw is Brumlet.`:a===`zh`?`${r?.cnName??`新系列`} 还在筹备中，当前可试玩 DROP 02：雾姆团。`:`${ee(r??n[n.length-1]).cnName??`A new series`} is still in planning; DROP 02: Brumlet is playable now.`,Q())}),V?.addEventListener(`click`,()=>{i=!i,V.textContent=o(i?s.twilloop.wished:s.twilloop.wish),V.classList.toggle(`active`,i),H&&(H.textContent=o(i?s.twilloop.wishedState:s.twilloop.idle)),T=i?a===`zh`?`Twilloop / 缝环团已加入本次浏览心愿；当前可试玩 DROP 02：雾姆团。`:`Twilloop has been added to this browsing session's wish list; DROP 02: Brumlet is playable now.`:a===`zh`?`已取消 Twilloop / 缝环团本次浏览心愿。`:`Twilloop has been removed from this browsing session's wish list.`,Q()}),et.forEach(e=>{e.addEventListener(`click`,()=>{qt(e.dataset.filter)&&(S=e.dataset.filter,Q())})}),Te?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-wish-id]`);if(!t?.dataset.wishId)return;y=t.dataset.wishId,b.wishSet=!0,h(),localStorage.setItem(de,y);let n=p(K());w=null,T=a===`zh`?`已将 ${n.title} 设为心愿款。`:`${n.title} is now your wishlist figure.`,Q()}),He?.addEventListener(`click`,t=>{if(!t.target.closest(`[data-random-wish]`))return;let n=e.filter(e=>e.id!==y),r=n[Math.floor(Math.random()*n.length)]??e[0];y=r.id,b.wishSet=!0,h(),localStorage.setItem(de,y),w=null;let i=p(r);T=a===`zh`?`已将 ${i.title} 设为心愿款。`:`${i.title} is now your wishlist figure.`,Q()}),I?.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`[data-claim-mission]`),r=t.closest(`[data-loop-reward]`);if(n?.dataset.claimMission){let e=n.dataset.claimMission,t=Lt().find(t=>t.id===e),r=b.claimedMissions.includes(e);t&&!r&&t.progress>=t.target&&(b.tokens+=t.reward,b.claimedMissions.push(e),h(),T=`${o(s.loop.missionDonePrefix)}${t.title}${o(s.loop.missionDoneMiddle)} ${t.reward} ${o(s.loop.missionDoneSuffix)}`,Q());return}r?.dataset.loopReward===`restock`&&b.tokens>=he&&(b.tokens-=he,b.stockBoost+=ge,h(),T=`${o(s.loop.restockedPrefix)} ${ge} ${o(s.loop.restockedSuffix)}`,Q())}),rt?.addEventListener(`click`,()=>{g={},_=[],x=[],v=0,b={tokens:0,duplicateInk:0,wishSparks:0,stockBoost:0,wishSet:!1,claimedMissions:[]},C=!1,w=null,T=a===`zh`?`已重置本地藏品。可以重新开盒。`:`Local collection reset. You can open boxes again.`,localStorage.removeItem(ce),localStorage.removeItem(le),localStorage.removeItem(ue),localStorage.removeItem(fe),Q()});var Xt=()=>{let e=window.location.hash.slice(1),t=e?document.getElementById(decodeURIComponent(e)):null;if(!t)return;let n=document.documentElement.style.scrollBehavior,r=document.querySelector(`.topbar`)?.getBoundingClientRect().height??0,i=t.getBoundingClientRect().top+window.scrollY-r-16;document.documentElement.style.scrollBehavior=`auto`,window.scrollTo({top:Math.max(0,i),behavior:`auto`}),window.setTimeout(()=>{document.documentElement.style.scrollBehavior=n},0)},$=()=>{let e=window.location.hash||`#top`;nt.forEach(t=>{t.classList.toggle(`active`,t.getAttribute(`href`)===e)})};Q(),$(),window.setTimeout(Xt,30),window.addEventListener(`hashchange`,()=>{$(),window.setTimeout(Xt,0)});